...
@RestController
@Path("/")
public class AddingController_OLD {
	...
	public Response add_customer_detail(@FormParam("model_id") String model_id,
		...
		try {
			ServiceCard_Pojo service_Card_Pojo = insert_Business_Interface.insertCustomerDetail(model_id, first_name,
	...
	public Response add_service_card(@FormParam("customer_detail_id") String customer_detail_id,
		...
		try {
			...
			List<ServiceParts_Pojo> service_Parts_Pojos = new ArrayList<ServiceParts_Pojo>();

			for (int i = 0; i < service_detail_Array.length; i++) {
				String value = service_detail_Array[i];
				ServiceParts_Pojo service_Parts_Pojo = mapper.readValue(value, ServiceParts_Pojo.class);
